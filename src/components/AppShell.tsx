...
interface NavItem { label: string; icon: typeof LayoutDashboard; badge?: string }

const navByRole: Record<RoleId, { group: string; items: NavItem[] }[]> = {
  EARLY_TALENT: [
    { group: 'Workspace', items: [{ label: 'My Dashboard', icon: LayoutDashboard }, { label: 'Integrated Curriculum', icon: BookOpen, badge: '2' }, { label: 'Pathway Selection', icon: Route }, { label: 'Program Overview', icon: GraduationCap }] },
    { group: 'Engineering Journey', items: [{ label: 'WF Course Assessments', icon: ClipboardCheck }, { label: 'Take Assessment', icon: Target }, { label: 'ASM Milestones', icon: GitBranch }, { label: 'ASM Fork & Environment', icon: Code2 }, { label: 'Advanced Intensives', icon: Sparkles }, { label: 'Architect Board', icon: Network }] },
  ...
  MENTOR_COACH: [
    ...
    { group: 'Guidance', items: [{ label: 'Pathway Panel', icon: Route }, { label: 'Waiver Recommendations', icon: ShieldCheck }, { label: 'Architect Board Panel', icon: Network }] },
  ],
  ENGINEERING_EXCELLENCE_COMMITTEE: [
    { group: 'Governance', items: [{ label: 'Cohort Overview', icon: Users }, { label: 'Pathway Panel', icon: Route }, { label: 'Assessment Setup', icon: Settings2 }] },
...
export function AppShell({ children, role, user, onRoleChange }: ShellProps) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[260px] flex-col bg-[#0c1b33] text-slate-300 transition-all duration-300 ${collapsed ? 'lg:w-[76px]' : ''} ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-5"><div className="space-y-6">{groups.map((group) => <div key={group.group}><p className={`mb-2 px-2 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-600 ${collapsed ? 'text-center' : ''}`}>{collapsed ? '•••' : group.group}</p><div className="space-y-1">{group.items.map((item, index) => <button key={item.label} onClick={() => setMobileOpen(false)} className={`group flex w-full items-center gap-3 rounded-md px-2.5 py-2 text-left text-[13px] font-medium transition-all ${index === 0 && group === groups[0] ? 'bg-blue-500/15 text-blue-300' : 'text-slate-400 hover:bg-white/5 hover:text-white'} ${collapsed ? 'justify-center px-0' : ''}`} title={collapsed ? item.label : undefined}><item.icon size={16} className="shrink-0" strokeWidth={1.8} />{!collapsed && <><span className="flex-1 truncate">{item.label}</span>{item.badge && <span className="rounded bg-blue-400/15 px-1.5 py-0.5 text-[10px] font-bold text-blue-300">{item.badge}</span>}</>}</button>)}</div></div>)}</div></nav>
