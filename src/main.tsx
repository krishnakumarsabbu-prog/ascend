...
function App() {
  ...
  return <AppShell role={role} user={user} onRoleChange={setRole}><Routes><Route path="/" element={role === 'EARLY_TALENT' ? <Dashboard data={dashboardQuery.data} isLoading={dashboardQuery.isLoading} /> : role === 'MENTOR_COACH' ? <MentorPortal user={user} /> : role === 'ENGINEERING_EXCELLENCE_COMMITTEE' ? <CommitteePortal /> : <SponsorPortal />} />
      <Route path="/committee" element={role === 'ENGINEERING_EXCELLENCE_COMMITTEE' ? <CommitteePortal /> : <Navigate to="/" replace />} /><Route path="/mentor" element={role === 'MENTOR_COACH' ? <MentorPortal user={user} /> : <Navigate to="/" replace />} /><Route path="/sponsor" element={role === 'SENIOR_LEADER_SPONSOR' ? <SponsorPortal /> : <Navigate to="/" replace />} /><Route path="/pathways" element={<PathwaySelection associateId={dashboardAssociateId} />} /><Route path="/asm" element={<AsmJourney associateId={dashboardAssociateId} />} /><Route path="/curriculum" element={role === 'EARLY_TALENT' ? <Curriculum /> : <Navigate to="/" replace />} /><Route path="/assessment/:courseId" element={role === 'EARLY_TALENT' ? <Assessment /> : <Navigate to="/" replace />} /><Route path="/assessment/result/:attemptId" element={role === 'EARLY_TALENT' ? <AssessmentResult /> : <Navigate to="/" replace />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes></AppShell>
}
